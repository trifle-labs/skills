# Trifle Labs Skills

Claude Code skills for monitoring, games, and automation.

## Skills

| Skill | Description | Install |
|-------|-------------|---------|
| [github-mentions](./github-mentions) | Monitor GitHub mentions across your orgs | `clawdhub install github-mentions` |
| [email-mentions](./email-mentions) | Monitor Gmail with security scanning | `clawdhub install email-mentions` |
| [got-it](./got-it) | Schelling point coordination game | `clawdhub install got-it` |

## Installation

Each skill can be installed individually via ClawdHub:

```bash
clawdhub install trifle-labs/skills/<skill-name>
```

Or clone the entire repo:

```bash
git clone https://github.com/trifle-labs/skills ~/.openclaw/workspace/skills/trifle-labs-skills
```

## License

MIT
